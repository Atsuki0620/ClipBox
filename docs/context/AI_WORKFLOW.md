...
| repo structure / archive 整理 | `python scripts/check_repo_boundaries.py`（OK）＋ §H。構造のみなら §F runtime スモークは `TESTING.md` §2 の条件で省略可 |
...
- **整理後は `python scripts/check_repo_boundaries.py` を必ず実行する**（archive 境界・ルート直下の荒れ・`docs/context/` 正本台帳の自動チェック。詳細は `REPO_STRUCTURE.md` §6。CI でも `Repository boundary guardrails` ジョブで実行される）。
