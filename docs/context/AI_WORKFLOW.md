...
- **ファイル移動 / リネーム / 分割は単独 PR で行い、挙動変更を混ぜない**（diff のレビュー可能性を確保）。
- **ルート `archive/`（`docs/archive/` とは別）は歴史資料**。`counter_service.py` / `history_repository.py` / `config_store.py` / `video_manager_methods.py` 等の死んだ Streamlit 期コードがある。**import・参照・復活は禁止**（§D / `SPEC_NEXTJS.md` §10/§12 の `is_judging`/`counters` と接続）。物理移動するなら「現行コードが対象を import していないこと」と「DB バックアップ」を確認してから。
- 構造整理 PR（挙動変更なし）のゲートは `TESTING.md` §2：`python -m pytest` 全緑 ＋ frontend は `npm run typecheck`/`lint` ＋ §F スモークで **before/after の挙動差分が無いこと** を確認・明記する。
