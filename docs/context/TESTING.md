...
| **Python 構文影響** | `python -m compileall api_app.py api core`（Windows/PowerShell でもワイルドカード展開不要。archived 旧 UI は対象外） |
...
python -m compileall api_app.py api core   # 構文チェック（Windows でも glob 不要）
