...
## 表示メタ・日付・バッジ用語（UI改修フィードバック）

UI改修フィードバック（`docs/nextjs-ui-renovation-feedback.md`）で使う表示用語。既存の DB/API フィールドを指すものと、UI改修の提案概念（実装前）を区別して記す。提案概念は本書を正本仕様として扱わない。

## 視聴日数 (view days)

APP_PLAYBACK 基準で、その動画を視聴した「日」の数（同じ日に複数回再生しても1日と数える）。
**視聴回数**（再生回数の総和）とは別の指標で、[総合スコア](#総合スコア--総合ランキング-composite)の因子（`視聴日数×1`）。
UI改修フィードバックでは全画面で視聴回数表示を主役から外し、視聴日数へ一本化する方針。視聴履歴そのものは削除しない（視聴日数の計算元データ）。

## 作成日 / 登録日 / 更新日

動画に紐づく3つの日付。混同しやすいため区別する。

| UI 表示 | DB/API カラム | 意味 | UI改修方針 |
|---|---|---|---|
| 作成日 | `file_created_at` | 動画ファイルの作成日時（`st_ctime` 由来）。 | カード/設定で `作成日` として表示する。 |
| 登録日 | `created_at` | ClipBox が DB に登録した日時。 | 作成日と混同しやすく運用優先度が低いため、UI から外す。 |
| 更新日 | `last_file_modified` | 動画ファイルの最終更新日時（`st_mtime` 由来）。 | 判定・選別でファイル名が変わると変化し得て誤解を招くため、UI から外す。 |

`作成日` は「動画ファイルの作成日」として扱い、`登録日` としては使わない。

## 判定日 / 選別日

**UI改修 提案（実装前）**。Tier1 の判定を行った日が「判定日」、Tier2 の選別を行った日が「選別日」。
設定画面のトグル名は `判定/選別日` でまとめ、カード表示では Tier1=`判定日` / Tier2=`選別日` と出し分ける。
表示に必要な値が既存 API で取れるかは実装時に確認する（不足する場合は別途仕様確認）。

## 該当Tierバッジ

**UI改修 提案（新設バッジ・実装前）**。動画カードに、そのカードがどの Tier 文脈か（`Tier1` / `Tier2`）を示すバッジ。
レベルは書かず Tier 名のみを表示し、Tier1 / Tier2 で色を分ける（具体色は統合 Variant K で全体テーマに合わせて決める）。
レベルはレベル選択ボタンや Tier 列で分かるため、バッジに重複表示しない。レベル表示対象（設定の `該当Tierのみ` / `Tier1・Tier2を両方表示`）とは別の設定。
