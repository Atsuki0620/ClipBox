...
### あとで見る (watch_later)
判定・選別を先延ばしするためのブックマーク。**DB 永続**（`videos.watch_later`、全端末共通）。
再生では解除されない。判定済み（level≥0）または選別完了（`+`）で自動解除。AVP候補とは別物。

### AVP候補 (avpCandidateIds)
AVP で並列再生するためにプールした動画群。**上限なし**。**localStorage**（`clipbox-avp`、そのブラウザのみ）。

### AVP再生対象 (avpPlayTargetIds)
AVP候補の中から、今回 AVP で実際に再生する動画。**最大4本**（`MAX_AVP_PLAY_TARGET=4`）。**localStorage**（`clipbox-avp`）。
...
ランキング種別の1つ。式: `round((視聴日数×1 + いいね×3) × (1 + 0.5×判定済み + 0.3×選別済み) × 100)`。
