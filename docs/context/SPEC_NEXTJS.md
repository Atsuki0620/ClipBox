...
| `/analysis` | 分析ダッシュボード | 旧分析 / 作業量・結果分布 / 視聴との関係 / 詰まり・次アクションの4タブ。期間別の視聴・判定トレンド、各種分布を表示する。詰まり・次アクションは read-only の候補一覧と既存画面への導線のみ。分析画面上の操作（再生・いいね・あとで見る・判定・選別）は後続段階で検討する。 | `app/analysis/page.tsx` |
