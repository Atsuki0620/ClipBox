...
API層 (FastAPI) - 現行 UI（Next.js）のバックエンド・core を共有（Phase 3 完了）
...
**重要**: Core層では`st`（Streamlit）をインポートしない（現行 UI は Next.js）。archived 旧 UI の `@st.cache_data` 関数は `archive/streamlit/ui/cache.py` に集約済み。

**API層（FastAPI）**: UI層と同じく core に一方向依存する。各ルーターは `core.app_service` のファサード経由でのみ
DB にアクセスし、`streamlit` を import しない。現行 UI（Next.js）のバックエンド（既定 8000）で、起動時 lifespan は read-only
（`init_database`/`run_startup_migration` は実行せず、DB 初期化・移行は起動スクリプト `run_api.bat`/`run_dev.bat` の `scripts/run_migrations.py`＋`scripts/startup_backup.py` が担う）。
...
**現行の DB 書き込み主体は Next.js + FastAPI**。archived 旧 Streamlit UI を起動して書き込む場合のみ、
Next.js + FastAPI 停止 + DB バックアップ前提で行う（SQLite 同時書き込み回避）。再生はサーバー機でプレイヤーが開く。
...
```
ClipBox/
├── api_app.py                # FastAPI エントリーポイント（現行 UI のバックエンド）
...
├── archive/streamlit/ui/     # 旧 Streamlit UI 層（archive 済み・現行導線ではない）
