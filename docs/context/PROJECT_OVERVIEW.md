...
| スキャン | ファイル利用可能性追跡 | ✅ 完了 |
| 再生 | 外部プレイヤー起動 | ✅ 完了 |
| 再生 | 視聴履歴自動記録（APP_PLAYBACK） | ✅ 完了 |
| 判定 | お気に入りレベル設定 | ✅ 完了 |
| 判定 | ファイルリネーム（プレフィックス変更） | ✅ 完了 |
| 判定 | 判定中バッジ表示 | ~~✅~~ **archived** |
| セレクション | !プレフィックス動画の管理 | ✅ 完了 |
| UI | Tier 1（一次判定）— ライブラリ/ランダム/運命の1本 | ✅ 完了 |
| UI | Tier 2（二次判定）— ライブラリ/ランダム/運命の1本 | ✅ 完了 |
| UI | ランキング | ✅ 完了 |
| UI | 分析ダッシュボード | ✅ 完了 |
| UI | AVP並列再生 | ✅ 完了 |
| UI | 設定タブ | ✅ 完了 |
...
```
サイドバー（radio）:
  Tier 1          - 一次判定。ライブラリ / ランダム / 運命の1本
  Tier 2          - 二次判定（セレクション）。ライブラリ / ランダム / 運命の1本
  ランキング       - 視聴回数・視聴日数・いいね数
  分析ダッシュボード - 視聴推移・判定推移・統計グラフ
  検索             - ファイル名・出演者検索
  AVP再生          - チェックした動画を最大4本並列再生
  設定             - ライブラリパス・プレイヤー設定
```
...
`?` prefix は旧資料・検討時の表記であり、現行仕様では使用しません。
...
| `FAVORITE_LEVEL_NAMES` | -1: 未判定, 0-4: Lv0-Lv4 | レベルの表示名 |

### 5.2 ユーザー設定（data/user_config.json）

ユーザー固有の設定を永続化するJSONファイル。`core/config_utils.py` で管理（`config_store.py` は archived）。

| キー | 説明 |
|------|------|
| `library_roots` | スキャン対象ディレクトリのリスト |
| `default_player` | デフォルトの動画プレイヤー（例: `"vlc"`） |
| `selection_folder` | セレクションフォルダのパス |
| `avp_exe_path` | Awesome Video Player の実行ファイルパス |
...
archived（Phase 1 で無効化。DB レコード自体は保持）:

| 方式 | viewing_method | 状態 |
|------|---------------|------|
| ファイルアクセス検知 | `FILE_ACCESS_DETECTED` | **archived** → `archive/detect_file_access.py` |
| 手動エントリー | `MANUAL_ENTRY` | **archived** → `archive/video_manager_methods.py` |
...
4. 判定ボタンをクリック
   ↓
5. ファイルリネーム（プレフィックス変更）
   ↓
6. DB更新（current_favorite_level, needs_selection=0, judgment_history INSERT）
   ↓
7. 画面自動更新（st.rerun(scope="fragment")）
```

※ is_judging フラグ操作（ステップ 2 の UPDATE is_judging=1 と完了時の is_judging=0）は Phase 1 でアーカイブ済み。DB カラムは保持。
...
**重要**: `scan_and_update()` はスキャンで見つからなかった全動画を `is_available=0` に更新する。スキャン済みディレクトリが0件の場合のみ更新をスキップ（安全ガード）。
...
| バッジ | 条件 | 色コード | 用途 |
|--------|------|----------|------|
| ~~判定中~~ | `is_judging = 1` | #f59e0b（アンバー） | **archived**（Phase 1）|
...
| 🎯 選別済み | `is_selection_completed = 1` | #22c55e（緑） | セレクション判定完了 |
