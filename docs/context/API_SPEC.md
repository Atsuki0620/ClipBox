...
- `is_judged`（bool）: ファイル名が `essential_filename` と異なるか（何らかのプレフィックスが付いているか）を示す派生値（`Video.is_judged()`）。業務用語の「判定済み」（`current_favorite_level >= 0`）とは混同しない。
...
### ソート / ページング方針
`view_count` / `last_viewed` は `viewing_history.viewing_method = APP_PLAYBACK` のみを集計する。
...
- `sort`: str — `favorite_level` | `creation_date` | `view_count` | `last_viewed` | `title` | `judged_at`。
  `judged_at` は Tier1 判定（`was_selection_judgment=0`）の最新判定日時で並べ、判定履歴の無い動画は
  `asc`/`desc` いずれでも常に末尾に置く（旧 `modified`=更新日ソートは廃止）。
...
**現行対応関数**: `VideoManager.get_videos()`（`core/video_manager.py`）。view_count/last_viewed/judged_at
ソートは `app_service` のマップ（`get_view_counts_map` / `get_last_viewed_map` / `get_latest_judged_at_map`）を併用。

---

### POST /api/videos/by-ids
**説明**: 指定IDの動画をまとめて取得する（AVP候補のバッチ取得用。候補が無制限でも 1 リクエスト）。
`/videos/{video_id}` より前に定義する（パス解決順）。

**リクエストボディ**: `{ "ids": int[] }`

**レスポンス**: `{ "items": Video[], "missing_ids": int[] }`（200 OK）。`items` は入力順を保持し
**削除済みも含む**。見つからなかったID（削除消滅など）は `missing_ids` に入れて返すため、クライアントは
localStorage 永続候補の掃除に使える。空配列は `items` 空・`missing_ids` 空。

**現行対応関数**: `app_service.get_videos_by_ids()`。
...
- `sort` / `page` / `page_size`: 一覧 API と同様。`judged_at` ソートは Tier2 選別判定
  （`was_selection_judgment=1`）の最新日時で並べる（Tier1 と Tier 別に切り替わる）。
...
**現行対応関数**: `api/avp.py:play_avp()`。AVP 起動後の視聴履歴は `app_service.record_avp_viewing()` が一括記録する。
...
**説明**: 全動画の全期間 APP 再生回数マップを返す（カード表示・ソート用）。
...
**説明**: 分析ダッシュボードの基礎データ（動画 + APP 再生の累積視聴回数）を期間・スコープ条件で返す。
...
**説明**: 指定期間・動画群の生視聴履歴を監査用に返す。集計APIと異なり `viewing_method` は絞らず、
`APP_PLAYBACK`、削除前の旧method、`NULL`、未知methodを含む。URL・パラメータ・レスポンス列は従来どおり。
...
`view_count` と `view_days` は `APP_PLAYBACK` のみを集計する。`likes` の集計定義は変更しない。
...
```json
{
  "library_roots": ["C:/videos", "D:/videos"],
  "default_player": "...",
  "avp_exe_path": "...",
  "db_path": "...",
  "selection_folder": "D:/selection",
...
`card_show_score` は廃止済み互換キー。設定ファイルやレスポンスに残り得るが、現行 UI のスコアバッジ表示には使わない。
...
`card_show_score` は廃止済み互換キーで、受け取っても現行 UI では使わない。
