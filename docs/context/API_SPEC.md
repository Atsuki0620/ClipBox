...
**Phase 3-B〜4-C でドメイン 30 エンドポイント（AVP 起動 API を含む）を実装済み**。加えて開発時用の
**運用エンドポイント 2 件（Runtime、末尾節参照）= 計 32** を提供する（実体は `api_app.py` + `api/`。並走中は起動時の
...
コントロール。ドメイン API ではなく**開発・運用補助**。実装は `api/runtime.py` →`core/runtime_control.py`（`psutil` で
ポート 8501/8000/3000 の LISTEN プロセス→PID を判定）。使い方は `docs/runtime-controls.md` を参照。
