...
ClipBox を **Python Flask（バックエンド API）+ Next.js（フロントエンド）** に移行する際の
HTTP API 設計仕様。現行 Streamlit UI が `core/` を呼び出している箇所を、Flask エンドポイントとして
再定義したもの。Flask は `core/` をそのまま再利用し、`ui/` と `streamlit_app.py` は破棄する。
...
| `get_kpi_stats`（Tier1 KPI） | `ui/components/kpi_display.py` | **UI 層にある（要 core 移設）** |

→ **移行時に `core/app_service.py` へ薄い wrapper 関数を追加**して上記を公開し、Flask は app_service 経由で呼ぶ。
各エンドポイントの「現行対応関数」には実体（`VideoManager.xxx` / `database.xxx`）を併記する。

### 実行環境前提（重要）
本アプリは**ローカル専用**である。Flask は動画ファイルへ直接アクセスできる**同一 Windows 環境**で動作する。
`POST /api/videos/{id}/play` 等のプレイヤー起動は **Flask 実行マシン上**で行われ、リモートのブラウザ端末ではない。
複数端末・リモート配信は想定しない。
...
一覧 API は `sort` / `page` / `page_size` を受け取り、ページング後の結果を返す。
...
- `sort`: str — `creation_date` | `favorite_level` | `view_count` | `last_viewed` | `title`（昇降は `_desc` 等で表現）。
- `page`: int（デフォルト 1） / `page_size`: int（デフォルト 100）。
...
**現行対応関数**: `ui/components/kpi_display.py:get_kpi_stats()`（**移行時に core へ移設が前提**）。
現行キャッシュは `ui/cache.py:get_kpi_stats_cached()`。
...
**クエリパラメータ**: `folder`: str（省略時は config のセレクションフォルダ）。
...
**レスポンス**: 動画ごとの集計レコード配列（snake_case JSON）。
...
**レスポンス**（snake_case に正規化。現行 DataFrame の日本語列名は使わない）:
```json
{ "items": [ { "rank": 1, "video": Video, "view_count": 42, "view_days": 10, "like_count": 5 } ] }
```
（`kind` に応じてスコア列が決まる）
...
{ "folder": "D:/selection" }
```
- `folder`: str — 省略時は config のセレクションフォルダを使用する。
...
**レスポンス**（200 OK）:
```json
{ "library_roots": ["C:/videos", "D:/videos"], "default_player": "...", "avp_exe_path": "...", "db_path": "..." }
```
...
**リクエストボディ**: 設定 dict（`library_roots` / `default_player` / `avp_exe_path` / `db_path`）。
