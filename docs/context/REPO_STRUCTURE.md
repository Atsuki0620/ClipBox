...
CI でも GitHub Actions の `Repository boundary guardrails` ジョブ（`.github/workflows/ci.yml`）として Pull request / `main` push 時に実行される。`python scripts/check_repo_boundaries.py --inventory` を付けると、未追跡・ignore 済みファイルの棚卸しを**件数のみ**で参考表示する（終了コードには影響しない）。`--inventory` は具体的なファイル名・パスを出力しない（リポジトリ相対パスでも動画名・個人情報を含み得るため）。詳細はローカルで `git status --short` / `git status --ignored --short` を人が確認し、出力を Pull request 本文やチャットに貼らない。
...
- **ルート直下の荒れ防止** — 一時 Notebook・DB コピー（`.db`/`.sqlite`/`.sqlite3` 本体と `*.db-wal`/`*.db-shm`/`*.db-journal` 等の SQLite サイドカー）・調査用ファイルを git 管理対象（tracked）に増やさない。Notebook・分析データを扱う場合は本書 §2・§5 の `docs/analysis/` 方針に従う（`docs/analysis/notebooks/`・`docs/analysis/data/` の公開可能資料は許可、ルート `notebooks/` は再作成しない）。
