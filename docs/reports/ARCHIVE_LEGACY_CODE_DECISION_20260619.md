...
現行の Next.js + FastAPI 実装領域から候補14件への参照は0件であり、`archive/` 直下の旧コードを `archive/legacy-code/` に隔離することは、AIによる現行コードとの誤認防止に有効である。ただし、今回の作業は本判断レポートの作成だけで停止し、物理移動は承認後の独立した構造整理として行う。
...
- 今回: ファイル移動、`CHANGELOG.md` 更新、コミット、Pull request作成、旧スクリプト実行、DBバックアップを行わない
...
| 候補 | 役割 | 現行参照 | docs参照区分 | archive内部の参照・依存 | 書き込み能力 | 判定 |
|---|---|---:|---|---|---|---|
...
承認後は、物理移動だけを扱う独立ブランチ `chore/archive-legacy-code-phase1` を作成し、次の12件だけを `git mv` で `archive/legacy-code/` 配下へ移動する。
...
`docs/context/PROJECT_OVERVIEW.md` には、移動後も `archive/config_store.py`、`archive/detect_file_access.py`、`archive/video_manager_methods.py` などの旧パスが残る。これは同ファイルがStreamlit期の歴史資料であるための既知・許容事項であり、リンク先の現配置を示す正本には使用しない。
...
本レポート作成時点では判断だけを確定し、物理移動には着手しない。旧スクリプトを一切実行せず、`data/` を変更しないことで安全を担保する。DBバックアップは、今回のdocs-only段階では作成しない。
