...
> 補足: `frontend/AGENTS.md` は**実在**（AI_WORKFLOW §A の参照は正しい）。WAL は未設定だが busy_timeout は `sqlite3` 既定（5秒）が効く点に注意（Streamlit 並走の同時書き込みは運用上禁止）。
...
| `core/database.py` に「WAL 未設定／busy_timeout は `sqlite3` 既定5秒が効く」コメント | 同時書き込み挙動の誤解を防ぐ | ほぼ無 | 中 |
...
- `core/database.py`: WAL 未設定／busy_timeout は `sqlite3` 既定5秒が効く旨のコメント。
