...
**✅ 解決済み (2026-06-03)** — Phase 1 でファイルアクセス検知機能ごと archived（`archive/detect_file_access.py`）。active 本体 `detect_recently_accessed_files` も `core/scanner.py` から除去済み
...
**✅ 解決済み (2026-06-03)** — Phase 1 でスナップショット機能ごと archived（`archive/snapshot.py`）。実行経路から外れたため負債解消
...
**✅ 解決済み (2026-06-03)** — `config_store.py` / `history_repository.py` を active tree から除去し `archive/` に退避（実体は `config_utils.py` / `database.py`）。`file_ops.py` は `create_file_scanner` の実利用があるため残置
...
**✅ 解決済み (2026-06-03)** — 表示名を `FAVORITE_LEVEL_NAMES`（`LvN` 表記）に一本化。未使用となった `models.level_to_display()` を削除し二重管理を解消
...
| A-7 | 🟢 低 | 📋 技術的負債 |
| A-8 | 🔴 高 | ✅ 解決済み (2026-06-03) |
| A-9 | 🟢 低 | 📋 技術的負債 |
| A-10 | 🟢 低 | ✅ 解決済み (2026-02-25) |
| B-1 | 🔴 高 | 📋 技術的負債 |
| B-2 | 🟡 中 | ✅ 解決済み (2026-02-25) |
| B-3 | 🟢 低 | ✅ 解決済み (2026-06-03) |
| B-4 | 🟡 中 | ✅ 解決済み (2026-02-25) |
| B-5 | 🟢 低 | 📋 技術的負債 |
| C-1 | 🔴 高 | ✅ 解決済み (2026-02-25) |
| C-2 | 🟡 中 | ✅ 解決済み (2026-02-25) |
| C-3 | 🟢 低 | ✅ 解決済み (2026-06-03) |
| C-4 | 🟡 中 | 📋 技術的負債 |
| C-5 | 🟢 低 | ✅ 解決済み (2026-06-03) |
| C-6 | 🔴 高 | ✅ 解決済み (2026-02-25) |
| C-7 | 🟢 低 | 📋 技術的負債 |
| C-8 | 🟡 中 | ✅ 解決済み (2026-02-25) |
| C-9 | 🟢 低 | ✅ 解決済み (2026-02-25) |

**合計**: 24件 / **✅ 解決済み**: 15件 / **📋 技術的負債**: 9件

> 2026-06-03 更新: Phase 1 整理の波及で A-8 / B-3 / C-3 / C-5 を解消。残る負債は A-2 / A-4 / A-5 / A-7 / A-9 / B-1 / B-5 / C-4 / C-7（いずれも個人利用規模では実害小、Flask 移行時／データ規模増で対応）。
