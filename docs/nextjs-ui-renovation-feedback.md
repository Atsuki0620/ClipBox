...
| ランキング | 集計画面 | あり（A/B/C） | UI LAB 作成済み。レポート: `frontend/src/app/lab/ranking/_review/COMPARISON.md`。採用判断はレビュー待ち |
| 分析 | ダッシュボード | あり（A/B/C） | UI LAB 作成済み。レポート: `frontend/src/app/lab/analysis/_review/COMPARISON.md`。採用判断はレビュー待ち |
| 検索 | 検索画面 | あり（A/B/C） | UI LAB 作成済み。レポート: `frontend/src/app/lab/search/_review/COMPARISON.md`。採用判断はレビュー待ち |
...
### ランキング
- **採用判断**: 3案（A カードランキング / B テーブル / C 上位カード＋下位テーブル）を UI LAB に作成。レポートの推奨は **案C を有力候補**（A＝操作重視 / B＝一望性重視）。**採用判断はユーザーレビュー待ち（未確定）**。レポート: `frontend/src/app/lab/ranking/_review/COMPARISON.md`。
- **未決事項**: 基線（A/B/C）/ テーブル行から操作可能にするか / 上位カードの本数 / モバイル時のテーブルの畳み方 / テーマ本採用。
- **改善要望**: 記録なし（レビュー後に追記）。
- **不具合メモ**: 記録なし（順位・スコアは UI 確認用ダミー。本体 APP_PLAYBACK 計算式ではない）。
...
### 検索
- **採用判断**: 3案（A 現状改善 / B Tier1・Tier2カード整合 / C 高機能フィルタ）を UI LAB に作成。レポートの推奨は **案A を基線＋案B のキャプション合流が有力**（C＝高機能フィルタは後段）。**採用判断はユーザーレビュー待ち（未確定）**。レポート: `frontend/src/app/lab/search/_review/COMPARISON.md`。
- **未決事項**: 基線（A / A+B / C）/ Tierキャプション常時表示 / Tier別グルーピング / 高機能フィルタの実装範囲 / 検索の作業台化（仕様変更）/ テーマ本採用。
- **改善要望**: 記録なし（レビュー後に追記）。
- **不具合メモ**: 記録なし（高機能フィルタはキーワード以外モック。検索結果は永続化しない）。
