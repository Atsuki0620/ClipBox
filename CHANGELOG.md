...
- ランキングの完全同点時の最終キーを `id ASC` に固定し、`availability=利用不可のみ`を含むAPI回帰テストを追加。
- 旧視聴履歴purgeの確認文字列・サービス稼働・DELETE後rollback・一時ファイルcleanup失敗をテストし、元のバックアップ処理エラーとcleanup失敗を併記するよう改善。
- ランキング表示範囲の3値ラベルを型で網羅し、「全動画」でも論理削除済みを除外する説明と、同期対象の`BACKUP_DIR`に関する運用確認を追加。API・DBスキーマ・migration・生履歴APIの契約は変更なし。
