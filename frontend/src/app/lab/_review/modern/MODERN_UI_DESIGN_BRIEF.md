...
![ex table](./crop-i1-table.png)
![ex table toolbar](./crop-i3-filters.png)

### 4-7. ランキング / 分布の見せ方
レベル分布・保存先内訳・総合ランキングは**横バー＋数値＋％のリスト**が軽量で読みやすい。
例:
![ex ranking](./crop-g2-ranking.png)

### 4-8. 設定画面
**縦サブナビ＋（ラベル＋入力＋helper text）**で整理。
例:
![ex settings nav](./crop-g3-nav.png)
![ex settings field](./crop-g3-field.png)
...
G の目標イメージ（KPI／サイドバー）:
![G target kpi](./crop-g1-kpi.png)
![G target sidebar](./crop-g1-sidebar.png)

H の目標イメージ（検索主役＋タグ的一覧）:
![H target](./crop-h2-table.png)

I の目標イメージ（フル機能テーブル）:
![I target](./crop-i1-table.png)
