...
- **現状**: `settings/SettingsBackupTab.tsx` に「手動バックアップ」ボタンがあり、履歴モックにも `trigger: "手動"` の行がある。これは scan-first の「スキャン前自動バックアップを主役にする」方針と明確に逆。
- **修正方針**: 手動バックアップボタンを撤去。履歴は「スキャン前（自動）」中心に寄せる。過去の手動履歴を残す必要がある場合は、旧データ表示であることを明記し、操作ボタンは置かない。
...
| 2-7 | ⚠️ | 検索の利用不可タイトル横バッジ | feedback 検索「利用不可はタイトル横バッジ＋行全体をグレーアウト」 | `search/SearchResults.tsx` は行薄表示＋操作 disabled は✅だが、タイトル横の「利用不可」バッジは無い。→ タイトルセルに利用不可バッジを追加 |
| 2-8 | ⚠️ | ランキング/検索の詳細フィルタの見た目統一 | feedback/計画「詳細フィルタ」 | ランキングはインライン常時表示、検索も全フィルタ常時表示。→ 開閉式の詳細フィルタとして揃える |
...
| 3-4 | ❓ サンプルDB接続前の API 束 | feedback 棚卸し H2「判定日/選別日マップ」「per-video 視聴日数」「任意動画集合の総合スコア＋総合順位」が必要。現状の統合 Variant K は合成データで表現しており、実 API 連携は未実装 | サンプルDB接続前の本体バックエンド作業として別 Pull request に分けるか。既存計算の流用が前提で、新規メトリクス定義はしない |
...
1. **§1（A 項目）** から着手: 設定の手動バックアップボタン撤去 → Tier1 カード/テーブル＋ページャ → ランキング/検索の Tier プルダウン → AVP 主要ボタン Tooltip 整理 → 検索 詳細フィルタ畳み＋保存先詳細列。
2. **§2（細部）** をまとめて反映（ボタン名・タイトル省略・詳細フィルタ畳み・SVG ロゴ・5列・AVP再生強調）。
3. **§3（要確認）** はユーザー回答後に対応方針を確定してから実装。サンプルDB接続前 API 束は UI 修正と混ぜず、本体作業として分ける。
...
- §3-4 サンプルDB接続前 API 束をどの Pull request に分けるか。
...
- **修正方針**: 2段 Popover（項目＋方向）に置換。項目は **レベル / 作成日 / 視聴日数 / タイトル / 判定日 の5項目**（最終再生日・視聴回数は入れない）。
...
- **レベル選択**:
  - 選択肢を **未・0・1・2・3・4** にする（`未`＝未判定へ戻すを含む。ユーザー回答 2026-06-28 で **未/0/1/2/3/4 の6択**に確定）。
  - レベルボタンの **左にある「判定」の文字ラベルは不要**（削除）。
  - **現状**: `tier1/Tier1CardActions.tsx` は「判定」ラベル＋ `Tier1LevelButtons`（`Lv0..Lv4` の5個・`未`なし）。→ ラベル削除し、選択肢を **未/0/1/2/3/4**（先頭に「未」を追加し Lv0 も残す6択）に変更。
  - **確定（ユーザー回答 2026-06-28）**: レベル選択は **未/0/1/2/3/4 の6択**（Lv0 を含む）。**未（未判定へ戻す）を Tier1 で出す**方針に変更し、統合計画 §9「Tier1 は未判定へ戻す操作を出さない」を本指摘で上書きする。
- **アクションボタン（横1段にまとめる）**:
...
- **優先**: §1-A の Tier1 ライブラリ改修に 7-A（フィルタ Popover）・7-B（並び替え2段 Popover）・7-D（レベル＝未/0/1/2/3/4・操作1段）を含めて実施 → 7-C（操作行の共通部品化）を Tier2/あとで見る/AVP へ展開。
- **確定（ユーザー回答 2026-06-28・要確認は解消済み）**:
  - 7-D: レベル選択は **未/0/1/2/3/4 の6択**（Lv0 を含む）。Tier1 で「未判定へ戻す」を出す（統合計画 §9 を上書き）。
  - 7-B: 並び替え項目は **視聴日数**（視聴回数は復活させない）。**最終視聴（最終再生日）は使わず作成日を優先**。
