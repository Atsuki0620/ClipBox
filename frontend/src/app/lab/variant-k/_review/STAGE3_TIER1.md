...
`/lab/variant-k/tier1` を、ライブラリ／ランダム／運命の1本の3タブを持つ完成度の高いモックに作り込んだ。段階2の統合シェル・サイドバー・Runtime control・寒色モダンテーマはそのまま維持。カード優先・視聴日数主役・サムネなしを徹底。
...
- `useTier1MockCardState.ts`（video id 単位でページ内共有するメモリのみのモック状態）
...
履歴セクションは作らない。大型「運命の1本を引く」ボタンを中心に据え、引いた1本のカードを表示。「最近見てない優先」トグルは **見た目のモックのみ**。**保持**は「このタブのセッション中だけ保持・タブを閉じると消える・履歴は残さない」という短い説明＋見た目で表現し、**実 sessionStorage には触れない**（保持仕様を撤去したようには見せない）。
...
- Lv0..Lv4 の判定操作は、モック上でも判定日を当日へ更新し、あとで見るを自動解除する。未判定フィルタ中なら対象カードは一覧から外れる。
...
- `watchLater` / `liked` / `avpCandidate` / `level` はカード単体ではなく Tier1 ページ内の video id 単位で共有する。ライブラリ/ランダム/運命の1本で同じ動画を操作しても状態は揃う。
- 操作ボタンは `aria-pressed` と状態に応じた `title` を持つ。カード上部のあとで見るバッジは `VariantKVideoCard.watchLater` prop でローカル状態と同期する。
...
- `full-fate.png` — 運命の1本タブ全体（大型「引く」・履歴なし・保持の説明・引いた1本）
