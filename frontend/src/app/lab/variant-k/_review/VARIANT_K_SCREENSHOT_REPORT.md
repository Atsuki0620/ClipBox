...
| 02 | Tier1 ライブラリ | カード（5列・既定） | [02-tier1-library-card](./variant-k-full-report-20260629/02-tier1-library-card.png) | 見出し右の横並びKPI・枠なしツールバー・上下ページャ（既定100）・5列・レベル6択・1段操作 |
| 03 | Tier1 ライブラリ | テーブル | [03-tier1-library-table](./variant-k-full-report-20260629/03-tier1-library-table.png) | 操作4列分割（再生/いいね/あとで見る/AVP候補）・列幅ドラッグ・上下ページャ |
| 04 | Tier1 ライブラリ | フィルタ Popover | [04-tier1-filter-popover](./variant-k-full-report-20260629/04-tier1-filter-popover.png) | 漏斗→レベル/保存先/状態/再生可/薄く・有効数バッジ |
| 05 | Tier1 ライブラリ | 並び替え Popover | [05-tier1-sort-popover](./variant-k-full-report-20260629/05-tier1-sort-popover.png) | 5項目＋降順/昇順の2段 |
| 06 | Tier1 ランダム | 多本提示 | [06-tier1-random](./variant-k-full-report-20260629/06-tier1-random.png) | 見出し/候補件数なし・シャッフル＋引き数 10/20/30（既定10）・5列 |
| 07 | Tier1 運命の1本 | 1本提示（全幅ワイド） | [07-tier1-fate](./variant-k-full-report-20260629/07-tier1-fate.png) | 枠なし・ボタン＋トグル横並び・全幅ワイドカード（メタ一段・操作一段） |
...
### 2. Tier1（2026-06-29 改修反映）

ヘッダは見出し「Tier1」＋右側に横一列 KPI（未判定/判定済み/判定率＋バー/今日の処理目安＋スパークライン）。KPI はタブ切替の上に1回だけ描画し、ライブラリ/ランダム/運命の1本の3タブで共通。

#### ライブラリ（カード）

![tier1 card](./variant-k-full-report-20260629/02-tier1-library-card.png)

- **見るべき点**: ツールバーは囲い枠なしの素配置（フィルタ漏斗／並び替え／カード⇄テーブル）。件数・1ページ件数（50/100/200・**既定100**）・前後送りをカードの**直上と直下**両方に配置。カードは**5列**（設定 表示タブの「カード列数」で増減・`31`）。各カードはレベル **未/0/1/2/3/4** の6択＋1段アイコン操作（再生＝アイコンのみ／♡＋数／栞＝あとで見る／`MonitorPlay`＝AVP候補）。
- **気になる点**: 17件のため 1/1。実データ件数でページ件数切替・前後送りの確認が要る。

#### ライブラリ（テーブル）

![tier1 table](./variant-k-full-report-20260629/03-tier1-library-table.png)

- **見るべき点**: タイトル/視聴日数/作成日/判定日/レベル（行内6択ボタン）に加え、操作を **再生／いいね／あとで見る／AVP候補 の4列に分割**。各列ヘッダ右端を**ドラッグして列幅を調整**できる。ページャはテーブルの直上・直下。
- **気になる点**: 列幅調整はメモリのみ（永続しない）。静的撮影のためドラッグ途中の見た目は本レポートでは未収録。
...
- **見るべき点**: 見出し・説明・候補件数テキストは置かない。引き数 **10/20/30（既定10）**＋シャッフルの行のみ。未判定かつ再生可能から多本提示。カードは5列・ライブラリと共通。

#### 運命の1本

![tier1 fate](./variant-k-full-report-20260629/07-tier1-fate.png)

- **見るべき点**: 見出し・補足テキスト・囲い枠を撤去。「運命の1本を引く」ボタンと「最近見てない優先」トグルを横一列に配置。引いた1本は**全幅ワイドカード**（バッジ＋タイトル→メタ一段→操作一段）で、縦を圧縮し横の情報量を増やす。表示項目とボタンはライブラリと共通。

#### 再生中ハイライト

![tier1 playing](./variant-k-full-report-20260629/08-tier1-playing-highlight.png)

- **見るべき点**: 押したカードが再生中（amber）ハイライト。バッジより優先。
...
- Tier1 ヘッダの横並び KPI＋枠なしツールバー＋上下ページャ（`02`・既定100・ページサイズ 50/100/200 と前後送りの粒度）。
- Tier1 カードの5列と、設定 表示タブ「カード列数」連動（`02`/`31`・既定5／範囲3–6・列幅の好み）。
- Tier1 テーブルの操作4列分割＋列幅ドラッグ調整（`03`・列順・初期幅・操作のしやすさ）。
- Tier1 ランダムの引き数 10/20/30（`06`・既定10）と運命の全幅ワイドカード（`07`・メタ一段/操作一段の情報密度）。
