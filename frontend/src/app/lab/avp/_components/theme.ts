// UIラボ「AVP画面」共通テーマ（寒色モダンコンソール＝Variant J と同一値）。
// 【役割】3案（A/B/C）のルート div に当てる CSS 変数上書きを一元化し、見た目比較の前提を揃える。
// 【設計制約】globals.css は変更しない。ルート要素の style にのみ適用する。
// 【依存関係】React の CSSProperties 型のみ。

import type { CSSProperties } from "react";

// クールニュートラル＋寒色アクセント（lab/tier1-random/variant-j・watch-later と同一 THEME）。
export const AVP_THEME: CSSProperties = {
