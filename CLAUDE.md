...
| ファイル | 説明 | 主な読者 |
|---------|------|---------|
| `PROJECT_OVERVIEW_20260125.md` | プロジェクト概要、設定、ビジネスルール | 全員 |
| `IMPLEMENTATION_GUIDE_20260125.md` | システム構造、機能実装マップ、データフロー | 開発者 |
| `DATA_MODEL_20260125.md` | データベース定義、テーブル構造、クエリ例 | 開発者 |
...
1. **新機能追加時**:
   - `IMPLEMENTATION_GUIDE`の機能実装マップを更新
   - 必要に応じて`PROJECT_OVERVIEW`のビジネスルールを追加

2. **データベーススキーマ変更時**:
   - `DATA_MODEL`のテーブル定義を更新
   - マイグレーション履歴に記録

3. **アーキテクチャ変更時**:
   - `IMPLEMENTATION_GUIDE`の構成図を更新
   - 該当するモジュール責務を修正

### AIへの読み込ませ方

AIにコンテキストを与える場合、以下の順序で読み込ませることを推奨：

1. `CLAUDE.md`（本ファイル）- 必須
2. `docs/PROJECT_OVERVIEW_20260125.md` - プロジェクト理解に必要
3. `docs/IMPLEMENTATION_GUIDE_20260125.md` - 実装作業時
4. `docs/DATA_MODEL_20260125.md` - DB関連作業時

### アーカイブドキュメント

`docs/`内の日付が古いドキュメント（例: `*_2026-01-24.md`）は参考資料としてアーカイブ対象です。最新情報は日付が新しいファイルを参照してください。
