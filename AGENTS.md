...
ClipBox は複数ストレージ（Cドライブ・外付けHDD）の動画ファイルを管理し、視聴履歴・お気に入りレベル・セレクションを追跡する**ローカル専用ツール**です。現行 UI は **Next.js + FastAPI**。旧 Streamlit UI は Phase 5 で `archive/streamlit/` へ archive 済みで、通常導線には含めません（削除はせず比較・退避用に保持）。

| サーバー | URL | 役割 |
|---|---|---|
| Next.js | `localhost:3000` | 新フロントエンド（現行 UI） |
| FastAPI | `localhost:8000` | バックエンド API（`api_app.py` + `api/`） |
| Streamlit | `localhost:8501` | 旧 UI（`archive/streamlit/` に archive 済み・通常停止） |
...
**歴史資料（正本ではない・現行と誤認しない）**: `docs/archive/PROJECT_OVERVIEW_STREAMLIT.md`（Streamlit 期の概要。旧 `docs/context/PROJECT_OVERVIEW.md`）、`docs/archive/`（移行作業の記録）、`docs/reports/`（日付付きレビュー）。
...
- API のみ: `run_api.bat` / Streamlit 旧 UI（archive 済み・通常使わない）: `archive/streamlit/run_clipbox.bat`
- Python 依存: `python -m pip install -r requirements.txt`
- Node 依存: `cd frontend && npm install`
- 単体テスト（バックエンド）: `python -m pytest`（`tests/` を自動検出）
- 静的チェック: `python -m compileall api_app.py api core`（Windows/PowerShell でもワイルドカード展開不要。archived 旧 UI は対象外）
...
1. `core/` に `import streamlit` しない（現行 UI は Next.js）。データ変更後のキャッシュ無効化は Next.js 側の TanStack Query `qc.invalidateQueries` で行う。archived 旧 UI の `@st.cache_data` は `archive/streamlit/ui/cache.py` に集約済み。
2. DB 接続は `get_db_connection()` 経由のみ（`sqlite3.connect()` 直呼び禁止）。
3. N+1 クエリ禁止（ループ内 DB 呼び出し禁止）。
4. 論理削除を尊重（クエリに `is_deleted = 0`）。
5. **書き込みは一方のサーバーのみ**で行う（WAL 未設定。同時書き込みは `sqlite3` 既定の約5秒のロック待ち後に `database is locked`／`SQLITE_BUSY` 相当で失敗し得る）。通常は Streamlit を起動しないため現行は Next.js + FastAPI 単独。archive 済みの Streamlit 旧 UI を起動して write 確認する場合は Next.js + FastAPI を停止し DB バックアップを取る。
