...
5. **書き込みは Next.js(FastAPI) と Streamlit のどちらか一方のみ**で行う（WAL 未設定。同時書き込みは `sqlite3` 既定の約5秒のロック待ち後に `database is locked`／`SQLITE_BUSY` 相当で失敗し得る）。Next.js の write 検証時は Streamlit を停止し DB バックアップを取る。
